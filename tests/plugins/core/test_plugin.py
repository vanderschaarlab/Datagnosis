...
class MockPlugin(Plugin):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.requires_intermediate = False
...
def test_mock_plugin_ok() -> None:
    ...
    assert plugin.requires_intermediate == False

    assert plugin.has_been_fit == False
    plugin.fit(
        DataHandler(
            torch.Tensor(
                [
    ...
    assert plugin.has_been_fit == True

    assert plugin.compute_scores() == [1]

    # plot different scores
    plugin.plot_scores(show=False)
    plugin._scores = np.asarray([1, 2])
    plugin.plot_scores(show=False)
    plugin._scores = np.asarray([1, 2, 3]), np.asarray([4, 5, 6])
    plugin.plot_scores(show=False)
    plugin._scores = np.asarray([1, 2])
    plugin.plot_scores(show=False, plot_type="scatter")
    ...
    plugin.plot_scores(show=False, plot_type="scatter", axis=0)
    plugin.plot_scores(show=False, plot_type="scatter", axis=1)
...
def test_mock_plugin_extract() -> None:
    plugin = MockPlugin(
        model=torch.nn.Linear(2, 1),
        criterion=torch.nn.MSELoss(),
        optimizer=torch.optim.Adam(torch.nn.Linear(2, 1).parameters(), lr=0.01),
...
def test_mock_plugin_fail_on_extract_by_threshold() -> None:
    ...
    with pytest.raises(ValueError):
        extracted = plugin.extract_datapoints(
            method="threshold", n=5, indices=[1, 2, 3, 4, 5]
        )
